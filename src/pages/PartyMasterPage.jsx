...
function FormField({ field, value, onChange, disabled, error, options, onAddNewOption, isLoadingPincode }) {
  ...
  return (
    <label className={`flex flex-col gap-2 ${field.wide ? 'md:col-span-2 xl:col-span-3' : ''}`}>
      ...
      {field.type === 'creatable-select' ? (
        <select
          ...
          onChange={(event) => {
            if (event.target.value === '__add_new__') {
              const newOption = prompt(`Enter new ${field.label}:`);
              if (newOption && newOption.trim() !== '') {
                if (onAddNewOption) onAddNewOption(newOption.trim());
                onChange(field.key, newOption.trim());
              }
            } else {
        ...
        >
          {value && !selectOptions.includes(value) && (
            <option value={value}>{value}</option>
          )}
          {selectOptions.map(option => (
            <option key={option} value={option}>{option}</option>
          ))}
          {!disabled && <option value="__add_new__" className="font-bold text-emerald-600">+ Add New {field.label.split(' ')[0]}</option>}
        </select>
      ) : field.type === 'select' ? (
        <select
          value={value || selectOptions[0] || ''}
...
function PartyMasterForm({ mode, party, onBack }) {
  const { addPartyMaster, updatePartyMaster, getNextPartyCode, partyCategories, addPartyCategory } = useERPStore();
  ...
  useEffect(() => {
    if (party) {
      ...
      const defaultCategory = partyCategories[0];
      setForm({ 
        ...EMPTY_PARTY, 
                  ...
                  {group.fields.map(field => {
                    ...
                    return (
                      <FormField
                        ...
                        options={field.key === 'partyCategory' ? partyCategories : isCityField && cityOptions.length > 0 ? cityOptions : field.options}
                        onAddNewOption={field.key === 'partyCategory' ? addPartyCategory : (newOption) => {
                          if (isCityField) setCityOptions(prev => [...prev, newOption]);
                        }}
