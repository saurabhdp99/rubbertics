...
function TransportMasterForm({ mode, transporter, onBack }) {
  const { addTransporterMaster, updateTransporterMaster, getNextTransporterCode } = useERPStore();
            ...
            <div className="flex flex-col xl:flex-row gap-4 items-center">
              ...
              <div className="flex flex-wrap gap-3 w-full xl:w-auto">
                <select
                  ...
                  onChange={event => setTransporterMasterTypeFilter(event.target.value)}
                ...
                >
                  {transporterTypes.map(type => <option key={type} value={type}>{type === 'All' ? 'All Transport Types' : type}</option>)}
                </select>
                <select
                  value={transporterMasterStatusFilter}
                  onChange={event => setTransporterMasterStatusFilter(event.target.value)}
                  className="px-4 py-3 text-sm rounded-xl text-slate-600 border border-slate-200 bg-white hover:bg-slate-50 focus:border-emerald-500/50 transition-all outline-none cursor-pointer"
                >
                  {statusTypes.map(status => <option key={status} value={status}>{status === 'All' ? 'All Status' : status}</option>)}
                </select>
              </div>
          ...
          <div className="glass-card rounded-2xl overflow-hidden shadow-2xl pb-4">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 pt-4 border-t border-slate-100">
              <div className="flex items-center gap-3 text-sm font-medium text-slate-500">
                ...
                <select
                  value={transporterMasterItemsPerPage}
                  onChange={event => setTransporterMasterItemsPerPage(Number(event.target.value))}
